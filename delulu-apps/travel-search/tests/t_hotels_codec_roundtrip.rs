...
fn test_roundtrip_internal_codec() {
    ...
    for case in &test_vectors.cases {
        ...
        let amenities: Vec<delulu_travel_search::Amenity> = case
            .input
            .amenities
            .as_deref()
            .unwrap_or_default()
            .iter()
            .filter_map(|a| delulu_travel_search::Amenity::from_str_name(a))
            .collect();
            ...
            Ok(mut params) => match params.generate_ts() {
                Ok(encoded) => match HotelSearchParams::from_ts(&encoded) {
                    Ok(_decoded) => {
                        ...
                        let checkin_matches =
                            params.checkin_date.contains(&case.input.checkin_date);
                        ...
                        let location_matches =
                            location_id_matches && coordinates_matches && display_name_matches;
                        ...
                        if let Some(ref amenity_names) = case.input.amenities {
                            for name in amenity_names {
                                if let Some(amenity) =
                                    delulu_travel_search::Amenity::from_str_name(name)
                                {
                            ...
                            && filters_match
                        {
                            ...
                            if !checkin_matches {
                                println!(
                                    "  Checkin: expected {}, got {}",
                                    case.input.checkin_date, params.checkin_date
                                );
                            }
                            if !checkout_matches {
                                println!(
                                    "  Checkout: expected {}, got {}",
                                    case.input.checkout_date, params.checkout_date
                                );
                            }
                            if !currency_matches {
                                println!(
                                    "  Currency: expected {}, got {}",
                                    case.input.currency, params.currency
                                );
                            }
                            if actual_adults != expected_adults {
                                println!(
                            ...
                            if !location_matches {
                                ...
                                println!(
                                    ...
                                    case.input.location_id, params.loc_ts_id
                                );
                                println!(
                                    "    coordinates: expected '{}', got '{}'",
                                    case.input.coordinates, params.loc_ts_coords
                                );
                                println!(
                                    "    display_name: expected '{}', got '{}'",
                                    case.input.display_name, params.loc_ts_name
                                );
                            }
                            if !filters_match {
