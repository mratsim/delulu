...
//! # MCP Server (library)
//!
//! Provides the [`WebsearchServer`] shared by the standalone
//! `delulu-websearch-mcp` binary and `delulu-all-mcp`.
//!
...
///
/// Shared by the standalone `delulu-websearch-mcp` binary and `delulu-all-mcp`.
///
/// Pre: constructed via [`WebsearchServer::new`] with an `Arc<EngineRegistry>`
/// and an `Arc<SessionCache>`.
...
impl WebsearchServer {
    ...
    ///
    /// Pre: `engine_registry` is an `Arc<EngineRegistry>` with the engines to
    /// search (e.g. `create_default_registry()`); `session_cache` is an
    /// `Arc<SessionCache>` holding pagination state.
    ...
    pub fn new(engine_registry: Arc<EngineRegistry>, session_cache: Arc<SessionCache>) -> Self {
        Self {
            engine_registry,
            session_cache,
...
impl WebsearchServer {
    ...
    async fn web_search(&self, params: Parameters<WebSearchInput>) -> Result<String, String> {
        ...
        let mut all_results: HashMap<String, Vec<crate::SearchResult>> = HashMap::new();
        let mut engine_errors: Vec<String> = Vec::new();
        let mut stored_continuation_engine: Option<String> = None;
        let mut continuation_box: Option<Box<dyn crate::Continuation>> = None;
