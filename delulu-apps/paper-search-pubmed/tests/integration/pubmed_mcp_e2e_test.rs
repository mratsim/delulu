...
fn find_binary() -> std::path::PathBuf {
    let ws = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).parent().unwrap().parent().unwrap().to_path_buf();
    for c in [ws.join("target").join("debug").join(BINARY_NAME), ws.join("target").join("release").join(BINARY_NAME)] {
...
fn check_output(output: std::process::Output) {
    ...
    assert!(output.status.success(), "Python tests failed (exit: {:?})", output.status.code());
}
...
async fn test_pubmed_mcp_e2e_stdio() {
    ...
    let output = run_python("test_pubmed_mcp_e2e_stdio.py", &[find_binary().to_string_lossy().to_string(), cfg_path.to_string_lossy().to_string()]).await;
...
async fn test_pubmed_mcp_e2e_http() {
    ...
    let fixture_url = format!("{}/entrez/eutils", url);
    let cfg = serde_json::json!({ "fixture_url": fixture_url });
    let cfg_path = std::env::temp_dir().join(format!("pubmed_e2e_http_{}.json", std::process::id()));
    std::fs::write(&cfg_path, cfg.to_string()).unwrap();

    let binary = find_binary();
    let port = get_free_port();
    let mut child = Command::new(&binary)
        .args(["--api-base-url", &fixture_url, "http", "--host", "127.0.0.1", "--port", &port.to_string()])
    ...
    loop {
        ...
        assert!(start.elapsed() < Duration::from_secs(10), "MCP server not ready");
        tokio::time::sleep(Duration::from_millis(100)).await;
    }

    let output = run_python("test_pubmed_mcp_e2e_http.py", &[port.to_string(), cfg_path.to_string_lossy().to_string()]).await;
